...
## 📊 View Workflows on Temporal UI (Like Airflow!)

### Quick Access
1. Open browser and navigate to: http://localhost:8080
2. Select namespace: `default`
3. View workflow list in **Workflows** tab

### Filter Workflows by Status (Like Airflow DAGs!)

**Filter by Status:**
- `OrderStatus = "FAILED"` → See all failed orders
- `OrderStatus = "COMPLETED"` → See all successful orders
- `OrderStatus = "RUNNING"` → See all running orders
- `OrderStatus = "CANCELLED"` → See all cancelled orders

**Filter by Current Step:**
- `CurrentStep = "PROCESS_PAYMENT"` → See orders stuck in payment
- `CurrentStep = "COMPENSATION"` → See orders being rolled back

**Filter by Amount:**
- `OrderAmount > 1000` → See high-value orders

**Filter by User:**
- `UserId = "user-123"` → See all orders for a user

### View Workflow Details

Click on any workflow to see:
- **Timeline**: Visual timeline showing all steps (like Airflow graph view)
- **Current Step**: See exactly which step is running
- **Status**: Real-time status (RUNNING, COMPLETED, FAILED, CANCELLED)
- **History**: Complete execution history
- **Logs**: Step-by-step logs with emojis for easy reading
- **Search Attributes**: OrderStatus, CurrentStep, OrderAmount, etc.

### Example: Find All Failed Orders

1. Go to **Workflows** tab
2. Click **Filters**
3. Add filter: `OrderStatus = "FAILED"`
4. See all failed workflows with error messages

See [UI_GUIDE.md](./UI_GUIDE.md) for detailed UI usage guide.
